...
return m1.getStartTime() - m2.getStartTime();
 ...
 course home
 Next up: Reverse String in Place
...
/**
 * Created by mashashtutman on 2019-05-27.
 */
public class MergeMeetings {



    boolean doMeetingsOverlap(Meeting first, Meeting second) {
    ...
    Meeting mergeTwoMeetings(Meeting first, Meeting second) {
        if (first == null) {
            return second;
        }
        if (second == null) {
        ...
        Meeting newMeeting = new Meeting(Math.min(first.start, second.start), Math.max(first.end, second.end));
        return newMeeting;
    }

    List<Meeting> mergeMeetings(Set<Meeting> meetingSet) {
        List<Meeting> sortedMeetings = meetingSet.stream().sorted(Comparator.comparingLong(Meeting::getStart))
                .collect(Collectors.toList());
        List<Meeting> result = new LinkedList<>();
      Meeting currentMeeting = null;
       for (int i = 0; i < sortedMeetings.size(); i++) {
           if (doMeetingsOverlap(currentMeeting, sortedMeetings.get(i))) {
               currentMeeting = mergeTwoMeetings(currentMeeting, sortedMeetings.get(i));
           }
           else {
               result.add(currentMeeting);
               currentMeeting = sortedMeetings.get(i);
           }
    ...
    public static void main(String[] args) {
        MergeMeetings mergeMeetings = new MergeMeetings();
        Meeting one = new Meeting(3, 5);
        Meeting two = new Meeting(2, 3);
        Meeting three = new Meeting(1, 3);
        Meeting four = new Meeting(9, 10);
        Set<Meeting> meetingSet = new HashSet<>();
        meetingSet.add(one);
        meetingSet.add(two);
        meetingSet.add(three);
        meetingSet.add(four);
        System.out.println(mergeMeetings.mergeMeetings(meetingSet));
